...
describe("WavelengthForm (React Wrapper)", () => {
  ...
  test("forwards rightButton props", async () => {
    const schema = z.object({ name: z.string() });
    render(<WavelengthForm schema={schema} rightButton={{ label: "Go", buttonProps: { id: "go-btn", disabled: true } }} />);
  ...
  test("renders buttons only when defined", async () => {
    ...
    rerender(<WavelengthForm schema={schema} leftButton={{ label: "Back" }} centerButton={{ label: "Center" }} rightButton={{ label: "Next" }} />);
    await new Promise((r) => setTimeout(r, 0));
  ...
  test("buttons emit events and apply props", async () => {
    ...
    const left = host.shadowRoot!.querySelector("wavelength-button#left") as HTMLElement;
    const center = host.shadowRoot!.querySelector("wavelength-button#center") as HTMLElement;
    const right = host.shadowRoot!.querySelector("wavelength-button#right") as HTMLElement;
  ...
  test("default button events trigger callbacks", async () => {
    ...
    await new Promise((r) => setTimeout(r, 0));
    const host = document.querySelector("wavelength-form")!;
    const [back, center, submit] = host.shadowRoot!.querySelectorAll("wavelength-button") as any;
